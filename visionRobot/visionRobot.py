...
class VisionRobot():
    ...
    def find_contour_cube(self, image, cube):
        filtered_image = self.apply_mask_image(image, cube)
        contour = None
        if cube.color is "black":
            contour = self.camera.find_contour_cube_black(filtered_image)
        elif cube.color is "white":
            contour = self.camera.find_square_cube_white(image)
        else:
            contour = self.camera.find_largest_contour_color(filtered_image)
        return contour
...
if __name__ == "__main__":
    ...
    while cap.isOpened():
        ...
        img_gray = camera.apply_polyline(img_gray)
        squares = camera._find_squares(img_gray)
        square = camera._filterSquares(squares)
        ...
        try:
            ...
            #     print is_centre
            cv2.drawContours(image,square,-1,(0,255,0),6)
